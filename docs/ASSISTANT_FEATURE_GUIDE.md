...
and ratio, follow-through, commitment, interruption, and actionable-next-step
...
reflection ratio, summary ratio, ask/action clarity, and recent coaching events
derived from stored feedback events. The API is bounded to recent events and
does not trigger model work.
